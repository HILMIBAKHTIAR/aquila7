...
$.fn.jqGrid = function( pin ) {
	...
	return this.each( function() {
		...
		if(this.tagName.toUpperCase() !== 'TABLE') {
			$.alert({
	          title: 'ALERT',
	          content: "Element is not a table",
	          icon: 'fa fa-warning',
	          theme: 'modern',
	          type: 'red'		                      
	      	});
		...
		if(document.documentMode !== undefined ) { // IE only
			if(document.documentMode <= 5) {
				$.alert({
		          title: 'ALERT',
		          content: "Grid can not be used in this ('quirks') mode!",
		          icon: 'fa fa-warning',
		          theme: 'modern',
		          type: 'red'		                      
		      	});
		...
		updatepager = function(rn, dnd) {
			...
			//------------------------------------------- START TAMBAHAN UNTUK WIDTH GRID-------------------------------------------//
				if((ts.p.id).indexOf("browse_") !== -1){
					
					if (! $( "#string_span" ).length ) {
					    $("body").prepend("<span id='string_span' hidden></span>");
					}

					// jumlah table row dan collumn
					var total_row_table = document.getElementById(ts.p.id).rows.length;
					var total_cell_table = document.getElementById(ts.p.id).rows[0].cells.length;

					for(var i = 0; i < total_cell_table; i++){
						var longestName = "";

						// cari huruf terpanjang dalam column
						for(var j = 0; j < total_row_table; j++){
							var data_cell_table = document.getElementById(ts.p.id).rows[j].cells[i].innerHTML;

							if( longestName.length < data_cell_table.length ){
								longestName = data_cell_table;
							}
						}

						// cari width dari huruf tersebut
						$("#string_span").html(longestName);
						var length_cell_table = $("#string_span").width() + 20;
						if(length_cell_table < 150){ length_cell_table = 150; }

						// ubah th dan td nya
						if( i >= 2 ){
							// ubah th
							var nama_id_table = ts.p.id+"_"+ts.p.colModel[i].name;
							$("#"+nama_id_table).css("width",length_cell_table);

							// ubah td
							for(var j = 0; j < total_row_table; j++){
								document.getElementById(ts.p.id).rows[j].cells[i].style.width = length_cell_table + "px";
							}
						}

					}
				}
			//------------------------------------------- END TAMBAHAN UNTUK WIDTH GRID-------------------------------------------//
...
var xmlJsonClass = {
	...
	toObj: function(xml) {
		...
		if (xml.nodeType === 1) {
			...
			$.alert({
	          title: 'ALERT',
	          content: "unhandled node type: " + xml.nodeType,
	          icon: 'fa fa-warning',
	          theme: 'modern',
	          type: 'red'		                      
	      	});
            ...
            return this.each(function(){
                ...
                var xmlConvert = function (xml,o) {
                    ...
                    if(xmlJsonClass.xml2json && $.jgrid.parse) {
                    	...
                    	$.alert({
				          title: 'ALERT',
				          content: "xml2json or parse are not present",
				          icon: 'fa fa-warning',
				          theme: 'modern',
				          type: 'red'		                      
				      	});
			...
			if ($.ui.multiselect ) {
				if(opts.msel === "multiselect") {
					if(!$.jgrid._multiselect) {
						...
						$.alert({
				          title: 'ALERT',
				          content: "Multiselect plugin loaded after jqGrid. Please load the plugin before the jqGrid!",
				          icon: 'fa fa-warning',
				          theme: 'modern',
				          type: 'red'		                      
				      	});
